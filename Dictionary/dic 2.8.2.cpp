...
void addword(hashtable *&tudien) {
	system("cls");
	cout << "Them tu moi... Updating... Nhan bat ky phim nao de quay tro lai...";
	addwordbox();
	getch();
	system("cls");
}
...
void detailword(hashtable *tudien, word *&hientai) { //Ham xuat ra man hinh thong tin chi tiet cua tu can tra
	...
	while(keyhit != 27) {
		if(hientai != NULL) {
    		...
    		else if(hientai->dongnghia != "" && keyhit == 9) synonym(tudien, search(tudien, hientai->dongnghia));
		}
		else {
			keyhit = 27;
		}
