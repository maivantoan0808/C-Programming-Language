...
short boxx = 15, boxy = 5, boxs = 20, boxw = 5; //Vi tri bat dau x, y cua box, box size, so tu hien thi trong khung
...
void searchbox() {
	...
	while(keyhit != 13) {  
		...
		else if(dem < boxs - 12 && ((keyhit > 96 && keyhit < 123) || (keyhit > 64 && keyhit < 91))) {
			dem++;
			intext += char(keyhit);
			gotoxy(boxx + 12, boxy + 1);
			cout << intext;
		...
		else if((keyhit > 96 && keyhit < 123) || (keyhit > 64 && keyhit < 91)) {
			dem++;
			intext += char(keyhit);
			gotoxy(boxx + 12, boxy + 1);
			for (int i = dem - boxs + 12; i < dem; i++) cout << intext[i];
