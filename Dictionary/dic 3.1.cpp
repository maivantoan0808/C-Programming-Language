...
short boxx = 9, boxy = 5, boxs = 40, boxw = 10; //Vi tri bat dau x, y cua box, box size, so tu hien thi trong khung
...
void printerror(short k) {
	...
	else if(k == 4) e = "Vui long nhap: \"n\" hoac \"v\"";
	setcolor(62);
...
void addword(hashtable *&tudien) {
	...
	for(int k = 1; k < 10; k++) {
		...
		while(keyhit != 13) {
			...
			if(keyhit == 13) if(checktext(tudien, text, k) != 0) {
				keyhit = 0;
...
void synonym(hashtable *tudien, word *hientai) {
	...
	while(keyhit != 27) {
		if(hientai != NULL) {
			...
			cout << "Loai tu: "; 
			if(hientai->loai == "n") cout << "Danh tu";
			else if(hientai->loai == "v") cout << "Dong tu";
...
void detailword(hashtable *tudien, word *&hientai) { //Ham xuat ra man hinh thong tin chi tiet cua tu can tra
	...
	while(keyhit != 27) {
		if(hientai != NULL) {
    		...
    		else if(keyhit == 83) confirmdel(hientai);
